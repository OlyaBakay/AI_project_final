...
class ImageDataset(data.Dataset):
    def __init__(self, eq_path, an_path, csv_path, lower_bound=0, upper_bound=1000, transform=None):
        ...
        eq_filenames = sorted(os.listdir(eq_path), key=lambda b: int(b[b.find("_") + 1:b.find(".")]))
        an_filenames = sorted(os.listdir(an_path), key=lambda b: int(b[b.find("_") + 1:b.find(".")]))
        ...
        with open(csv_path) as f:
            df = pd.read_csv(f)
            self.data_labels = df["AnswerIsCorrect"]
            self.answers = df["CorrectAnswer"]
